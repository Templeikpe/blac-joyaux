...
<section class="flex items-center justify-center px-6 py-16">
    <div class="bg-white rounded-2xl shadow-sm p-8 w-full max-w-md text-center">
        ...
        <p class="text-sm text-gray-500 mb-6">
            Un code à 6 chiffres a été envoyé à <strong>{{ $client->email }}</strong>
        </p>
        ...
        <form method="POST" action="{{ route('verification.connexion.verifier') }}" class="space-y-4">
            @csrf
        ...
        <form method="POST" action="{{ route('verification.connexion.renvoyer') }}" class="mt-4">
            @csrf
